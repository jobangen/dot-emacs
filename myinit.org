...
(defun capture-report-date-file (path)
  (let ((name (read-string "Name: ")))
    (expand-file-name
     (concat path (format-time-string "%Y-%m-%d-%H%M ") name ".txt"))))
...
"* TODO %^{Task} :%^{Category|wiss|arbeit|pers}:%^G
SCHEDULED: %t
:PROPERTIES:
:CREATED: %U
:CATEGORY: %\\2
...
("j" "Journal entry" entry (function org-journal-find-location)
                               "* %(format-time-string org-journal-time-format)%^{Title}%? :journal:
:PROPERTIES:
:CREATED: [%<%Y-%m-%d %a %H:%M>]
...
("m" "mail todo" entry (function org-journal-find-location)
 "* TODO %^{Task} :%^{Category|wiss|arbeit|pers}:@mail:
SCHEDULED: %t
:PROPERTIES:
:CREATED: %U
:CATEGORY:  %\\2
...
(setq org-refile-targets (quote ((("~/Dropbox/db/org/pers.org") :maxlevel . 3)
