...
("a" "Appointment" entry (file (orgdir-expand "calender.org"))
 "* %^{Description}
<%(org-read-date)%?>
\n")
...
("j" "Journal")
("jj" "Journal" entry (file+datetree+prompt (orgdir-expand "journal.org"))
 "* %^{Headline}  :journal:
:PROPERTIES:
:CREATED: %U
:CATEGORY:  %^{Category|wiss|work|pers}
...
("jl" "Literaturbearbeitung" entry (file+datetree (orgdir-expand "journal.org"))
 "* Literaturbearbeitung
:PROPERTIES:
:CATEGORY: wiss
:END:" :jump-to-captured t :immediate-finish t)

("jt" "Journal todo" entry (file+datetree+prompt (orgdir-expand "journal.org"))
"* TODO %^{Task}
SCHEDULED: %t
:PROPERTIES:
:CREATED: %U
...
("ja" "BerlinAntiquariat" entry (file+datetree+prompt (orgdir-expand "journal.org"))
 #'org-capture-berlinantiquariat :jump-to-captured t)

("jw" "Wissenschaft" entry (file+datetree+prompt (orgdir-expand "journal.org"))
...
("z" "Zettel" plain (file (capture-report-date-file  "~/Dropbox/db/zk/"))
 "#+TITLE: %^{Title}
#+DATE: %U\n* Schlagwörter
tags:\n* Inhalt
%?\n* Literatur\n
...
(defun zettelkasten-combine-parse-values ()
  (interactive)
  (shell-command "cat ~/Dropbox/db/zk/*.txt > ~/Dropbox/db/zk/stats/zettel-combined.txt")
  (find-file "~/Dropbox/db/zk/stats/zettel-combined.txt")
