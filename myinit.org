...
;;  :bind ("C-k" . viking-kill-thing-at-point)
  :config
  (viking-global-mode)
  (setq viking-greedy-kill nil)
  (setq viking-enable-region-kill t)
  (setq viking-enable-blinking nil)
  (setq viking-kill-functions (list 'viking-kill-line-from-point
                                    'viking-kill-line
                                    'viking-kill-paragraph
                                    'viking-kill-buffer))
 (bind-key "C-k" 'viking-kill-thing-at-point viking-mode-map)
 (bind-key "C-d" 'delete-char viking-mode-map)
)
#+end_src
...
(use-package org
  ...
  :bind (("C-c l" . org-store-link)
         ...
         ("C-c c" . org-capture)
         ("C-c ä" . org-capture)
         ("<f11>" . job/org-clock-in-everywhere))
  :config
   (progn
     ;;Pfade
...
#+begin_src emacs-lisp
(bind-key "C-c 0" '(lambda () (interactive) (find-file "~/.emacs.d/myinit.org")))
...
;;  (defun job/clock-in-switch-state (state)
;;    "Switch a task to NEXT when clocking in. Skips capture tasks,
;;    projects, ideas and appointments."
;;     (when (not (and (boundp 'org-capture-mode) org-capture-mode))
;;        (cond
;;         ((and (member (org-get-todo-state) (list "TODO" "DONE" "WAITING" "HOLD" "DELEGATED" "CANCELLED")))
;;          "NEXT"))))
...
#+begin_src emacs-lisp
  ...
  (defun job/org-clock-in-everywhere (arg)
    (interactive "p")
...
;;https://github.com/mattfidler/my-emacs-startup/blob/master/startup-org.org
  ...
  (defvar bh/organization-task-id "5b1d075d-8ef1-4646-a598-27b59b2c7b9e")
...
#+begin_src emacs-lisp
  ...
  (bind-key "C-z" 'job/other-window-or-my/hsplit-last-buffer)
#+end_src
