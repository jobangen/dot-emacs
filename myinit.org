...
("jm" "Mailbearbeitung" entry (file+datetree (expand-dir-org "journal.org"))
 "* Mailbearbeitung
:PROPERTIES:
:CATEGORY: wiss
:END:" :jump-to-captured t :immediate-finish t)

("jb" "BerlinAntiquariat" entry (file+datetree+prompt (expand-dir-org "journal.org"))
 #'org-capture-berlinantiquariat :jump-to-captured t)

("je" "Electricity" plain (file "~/Dropbox/db/plot/electricity.csv")
"%(org-read-date); %?")
...
("gm" "Miete" plain (file (expand-dir-db "money.dat"))
 "%(org-read-date) * Unter den Eichen
    Expenses:Wohnung:Grundmiete               401,25€
    Expenses:Heizkosten                        91,00€
    Expenses:Betriebskosten                   103,00€
    Assets:Giro" 
    :immediate-finish t)

("gn" "Netflix" plain (file (expand-dir-db "money.dat"))
...
("z" "Zettel" plain (file (capture-report-date-file  (expand-dir-db "zk/")))
 "#+TITLE: %^{Title}
#+DATE: %U\n* Schlagwörter
tags:\n* Inhalt
%?\n* Literatur\n
...
(setq org-refile-targets (quote ((("pers.org") :maxlevel . 3)
                                 (("wiss.org") :maxlevel . 3)
                                 (("irw.org") :maxlevel . 3)
                                 (("~/Dropbox/diss/diss.org") :maxlevel . 3)
                                 (("antiq.org") :maxlevel . 1))))
...
(define-key ivy-minibuffer-map (kbd "C-#") 'zettelkasten-insert-regex)
;(defun zettelkasten-insert-regex ()
;  (interactive)
;  (insert "[a-z1-9]"))
...
(defun zettelkasten-markup ()
   (highlight-regexp "\\s-[1-9]*~" 'zettelkasten-lvl1-face)
   (highlight-regexp "[1-9]*-[1-9]*~" 'zettelkasten-lvl2-face)
   (highlight-regexp "\[1-9a-z\\-\]*~" 'zettelkasten-id-face))
