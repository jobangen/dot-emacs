...
** dir-setup
#+begin_src emacs-lisp
(defun expand-dir-dropbox (name)
  (expand-file-name name "~/Dropbox"))

(defun expand-dir-db (name)
  (expand-file-name name "~/Dropbox/db"))

(defun expand-dir-org (name)
  (expand-file-name name "~/Dropbox/db/org"))

(defun expand-dir-zk (name)
  (expand-file-name name "~/Dropbox/db/zk"))
...
** Pfade
#+begin_src emacs-lisp
(setq org-directory (expand-file-name (expand-dir-org "")))
(setq org-default-notes-file (expand-dir-org "inbox.org"))
;;(setq org-agenda-files '((expand-dir-org "")))
(setq org-agenda-diary-file (expand-dir-org "journal.org"))
...
("a" "Appointment" entry (file (expand-dir-org "calender.org"))
 "* %^{Description}
<%(org-read-date)%?>
\n")

("t" "todo" entry (file (expand-dir-org "inbox.org"))
 "* TODO %^{Task} %^G
SCHEDULED: %t
:PROPERTIES:
:CREATED:   %U
...
("n" "note" entry (file (expand-dir-org "inbox.org"))
 "* %^{Note}
:PROPERTIES:
:CREATED: %U
:END:
...
("i" "interruption" entry (file (expand-dir-org "inbox.org"))
 "%^{Task}
:PROPERTIES:
:CREATED: %U
:CATEGORY:  %^{Category|wiss|work|pers}
...
("j" "Journal")
("jj" "Journal" entry (file+datetree+prompt (expand-dir-org "journal.org"))
 "* %^{Headline}  :journal:
:PROPERTIES:
:CREATED: %U
:CATEGORY:  %^{Category|wiss|work|pers}
...
("jl" "Literaturbearbeitung" entry (file+datetree (expand-dir-org "journal.org"))
 "* Literaturbearbeitung
:PROPERTIES:
:CATEGORY: wiss
:END:" :jump-to-captured t :immediate-finish t)

("jt" "Journal todo" entry (file+datetree+prompt (expand-dir-org "journal.org"))
"* TODO %^{Task}
SCHEDULED: %t
:PROPERTIES:
:CREATED: %U
...
("ja" "BerlinAntiquariat" entry (file+datetree+prompt (expand-dir-org "journal.org"))
 #'org-capture-berlinantiquariat :jump-to-captured t)

("jw" "Wissenschaft" entry (file+datetree+prompt (expand-dir-org "journal.org"))
 #'org-capture-wiss :jump-to-captured t)

("m" "mail todo" entry (file+headline (expand-dir-org "inbox.org") "Mail")
 "* TODO %^{Task}
SCHEDULED: %t
:PROPERTIES:
:CREATED: %U
...
("b" "bibtex" plain (file (expand-dir-db "biblio.bib"))
"%?" :jump-to-captured t :empty-lines 1)

("l" "literature" entry (file+headline (expand-dir-org "inbox.org") "Literatur")
...
("Z" "Zettel TODO" entry (file+headline (expand-dir-zk "zk-todo.txt") "Inhalt")
 "* TODO %^{Task}
:PROPERTIES:
:CREATED: %U
:CATEGORY: wiss
...
(setq org-refile-targets (quote (((expand-dir-org "pers.org") :maxlevel . 3)
                                 ((expand-dir-org "wiss.org") :maxlevel . 3)
                                 ((expand-dir-org "irw.org") :maxlevel . 3)
                                 ((expand-dir-dropbox "diss.org") :maxlevel . 3)
                                 ((expand-dir-org "antiq.org") :maxlevel . 1))))
...
:config
 (progn
   (require 'org-ref)
   (setq org-ref-bibliography-notes (expand-dir-zk "")
      org-ref-default-bibliography '((expand-dir-db "biblio.bib"))
      org-ref-pdf-directory (expand-dir-texte ""))
  ...
  (let*
   ((key (bibtex-completion-normalize-candidate key))
    ...
    (ref (pcase (downcase (bibtex-completion-get-value "=type=" entry))
           ...
           ("inproceedings"
            (s-format
             "${author} ${date}. ${title}. ${subtitle}. In: ${editor}, ${booktitle} (pp. ${pages}). ${location}: ${publisher}."
             'bibtex-completion-apa-get-value entry))
           ("book"
            (s-format
...
(defun bibtex-completion-candidates-formatter (candidates width)
  ...
  (cl-loop
   ...
   if (assoc-string "author" entry 'case-fold)
     for fields = '("author" "title" "date" "=has-pdf=" "=has-note=" "=type=")
   else
     for fields = '("editor" "title" "date" "=has-pdf=" "=has-note=" "=type=")
...
;;  :ensure t
  :bind ("<f5>" . helm-bibtex)
        ("C-<f5>" . helm-resume)
  :config
   (setq helm-bibtex-bibliography '((expand-dir-db "biblio.bib")))
   (setq helm-bibtex-library-path (expand-dir-texte ""))
   (setq helm-bibtex-notes-path (expand-dir-zk ""))
...
(use-package org-gcal
  ...
  (setq org-gcal-client-id "553301842275-clecdgmr7i8741e3ck5iltlgfk3qf79r.apps.googleusercontent.com"
      ...
      org-gcal-file-alist '(("jobangen@googlemail.com" . (expand-dir-org "calender.org")))))
...
(use-package howm
  :diminish howm-mode
  :config
   (setq howm-directory (expand-dir-zk ""))
...
;;(add-hook 'text-mode-hook 'zettelkasten-parse-tags-values)
...
(defun zettelkasten-combine-parse-values ()
  (interactive)
  (shell-command "cat ~/Dropbox/db/zk/*.txt > ~/Dropbox/db/stats/zettel-combined.txt")
  (find-file (expand-dir-db "stats/zettel-combined.txt"))
  (zettelkasten-parse-tags-values))

(add-hook 'after-init-hook 'zettelkasten-combine-parse-values)
